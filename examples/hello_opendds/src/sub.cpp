...
class HelloListener : public virtual OpenDDS::DCPS::LocalObject<DDS::DataReaderListener> {
	public:
		...
		void on_data_available(DDS::DataReader_ptr reader) {
			HelloWorldData::MsgDataReader_var reader_i = HelloWorldData::MsgDataReader::_narrow(reader);
			...
			for(CORBA::ULong i = 0; i < message.length(); i++) {
				if (info[i].valid_data) {
					...
					std::cout << "userId: " << message[i].userId			<< std::endl;
					std::cout << "message: " << message[i].message.in()		<< std::endl;
				}
			}
